  ...
  switch (toolName) {
    ...
    case "get_accounts": {
      ...
      return accounts.map(a => {
        ...
        let line = `${sanitizeForPrompt(a.name)} (${a.type}): ${sign}${formatMoney(a.balance)}`;
    ...
    case "get_spending_summary": {
      ...
      const rawRows = db.prepare(
        `SELECT category, SUM(amount) as total, COUNT(*) as count FROM transactions
         ...
         AND (category IS NULL OR category NOT IN ('TRANSFER_OUT', 'TRANSFER_IN', 'LOAN_PAYMENTS'))${extraSql}
         GROUP BY category ORDER BY total DESC`
      ).all(start, end, ...extraParams) as { category: string | null; total: number; count: number }[];
      ...
      if (rawRows.length === 0) return `No spending found for that period${scope ? ` (${scope})` : ""}.`;
    ...
    case "get_recurring": {
      ...
      if (manual.length > 0) {
        ...
        for (const m of manual) {
          ...
          lines.push(`- ${sanitizeForPrompt(m.name)}: ${formatMoney(m.amount)}${day}${m.type ? ` [${sanitizeForPrompt(m.type)}]` : ""}`);
        }
      }
      return lines.join("\n");
    }
