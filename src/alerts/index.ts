...
export function generateAlerts(db: Database): Alert[] {
  ...
  for (const tx of large) {
    ...
    alerts.push({
      ...
      severity: "warning",
      message: `Large charge: ${formatCurrencyAmount(tx.amount, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} at ${sanitizeForPrompt(tx.merchant_name || tx.name)} (${sanitizeForPrompt(tx.account_name)})`,
      data: tx,
    });
  }
  ...
  for (const acct of lowBal) {
    alerts.push({
      ...
      message: `Low balance: ${sanitizeForPrompt(acct.name)} has ${formatCurrencyAmount(acct.current_balance, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
      data: acct,
    });
  }
